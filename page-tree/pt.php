...
if(in_array($user_role_cpt,$userrole_exp))
$checkuserrole=true;
else 
$checkuserrole=false;
		...
		$pages_chiled = get_pages($args_child); 
			...
			if (count($pages_chiled) || (isset($_GET['action']) && isset($_GET['eaction']) && $_GET['action'] == 'edit' && $_GET['eaction'] == 'create_sub' && $parentID == $thispageid ) ) {
				...
				if (count($pages_chiled)){
					...
					if($checkuserrole)
						echo '<span class="dd-handle" style="width:100%;margin-right: 1px;" ><a href="'.get_page_link( $page_chiled->ID ).'">'.$page_chiled->post_title.'</a></span>';
					else 		
						echo '<span  style="width:100%"><a href="'.get_page_link( $page_chiled->ID ).'">'.$page_chiled->post_title.'</a></span>';	
			...
			echo "<div class='dd' id='nestableMenu'>\n\n";
				echo "<ol class='dd-list'>\n";
                       ...
                       foreach ( $pages_main as $page_main ) {
						...
						if($checkuserrole)
						echo '<span class="dd-handle" style="width:100%;margin-right: 1px;" ><a href="'.get_page_link( $page_main->ID ).'">'.$page_main->post_title.'</a></span>';
						else 
						echo '<span  style="width:100%"><a href="'.get_page_link( $page_main->ID ).'">'.$page_main->post_title.'</a></span>';
